...
double _simulateAcceleration() {
  ...
  void _startJumping() async {
    ...
    setState(() {
      ...
      double newLat = 37.7749 + (Random().nextDouble() - 0.5) * 0.0001;  // losowa zmiana szerokości geograficznej
      double newLng = -122.4194 + (Random().nextDouble() - 0.5) * 0.0001;  // losowa zmiana długości geograficznej
      ...
      if (_isJumping(AccelerometerEvent(0.0, accelerationY, 0.0, DateTime.now()))) {
        _onJumpDetected();
      }
    });
  });
