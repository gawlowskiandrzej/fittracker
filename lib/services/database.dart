...
class DatabaseService {
  ...
  Future<List<Activity>> fetchRecentActivities() async {
    ...
    activity_types = {
      for (var doc in typesSnapshot.docs)
        (doc.data())['id'].toString(): (doc.data())['type'] ?? 'Nieznany',
    };
  ...
  Future<Map<String, int>> getActivityCounts() async {
    ...
    final activityValues = {
      for (var doc in typesSnapshot.docs)
        (doc.data())['type'].toString(): (doc.data())['id'],
    };
  ...
  Future<Map<String, int>> getDailyActvityCounts() async {
    ...
    final typesSnapshot =
        await activityCollection.where('userId', isEqualTo: user.uid).get();
    ...
    for (var doc in typesSnapshot.docs) {
      ...
      final weekday = DateFormat(
        "EEEE",
      ).format(dateTime); // Get the full name of the weekday
  ...
  Future<List<Activity?>> getStatistics() async {
    final user = FirebaseAuth.instance.currentUser;
    if (user == null)
      return [Activity.empty(), Activity.empty(), Activity.empty()];
    final typesSnapshot = await activityTypesCollection.get();
    activity_types = {
      for (var doc in typesSnapshot.docs)
        (doc.data())['id'].toString(): (doc.data())['type'] ?? 'Nieznany',
    ...
    final mostCaloriesBurnedSnapshot =
        await activityCollection
            .where('userId', isEqualTo: user.uid)
  ...
  Future<void> updateActivityStats(
    ...
    try {
      final activityStatsRef = FirebaseFirestore.instance.collection(
        'activity_stats',
      );

      // Pobierz obecne dane statystyk użytkownika
      final querySnapshot =
          await activityStatsRef.where('userId', isEqualTo: userId).get();
